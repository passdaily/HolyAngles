package info.passdaily.holy_angles_staff.typeofuser.common_staff_admin.zoom_scheduled
...
class ZoomScheduledViewModel(private val mainRepository: MainRepository) : ViewModel() {
    ...
    fun getMeetingListByAdmin(accademicId : Int,adminId: Int,schoolId :Int) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = mainRepository.getMeetingListByAdmin(accademicId,adminId,schoolId)))
        } catch (exception: Exception) {
            Log.i(TAG, "exception $exception")
            emit(Resource.error(data = null, message = exception.message ?: "Error Occurred!"))
        }
