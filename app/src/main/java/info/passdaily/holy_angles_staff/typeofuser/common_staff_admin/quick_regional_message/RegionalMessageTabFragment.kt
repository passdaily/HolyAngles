...
import info.passdaily.holy_angles_staff.databinding.FragmentAttendedTabBinding
...
class RegionalMessageTabFragment(
    var regionalMessageClickListener :RegionalMessageClickListener) : Fragment(), MessageTabClicker {

    var TAG = "RegionalMessageTabFragment"
...
//    lateinit var bottomSheetDetails : BottomSheetMessageDetails
    lateinit var bottomSheetRegionalMessageDetailsAdmin: BottomSheetRegionalMessageDetailsAdmin
////
    lateinit var bottomSheetUpdateMessage : BottomSheetUpdateRegionalMessage
...
//        bottomSheetDetails = BottomSheetMessageDetails()
        bottomSheetUpdateMessage = BottomSheetUpdateRegionalMessage()
        bottomSheetRegionalMessageDetailsAdmin = BottomSheetRegionalMessageDetailsAdmin()
    }
                        ...
                        R.id.menu_video -> {
                            ...
                            builder.setMessage("Are you sure want to delete template?")
                                ...
                                .setPositiveButton("Yes") { _, _ ->
                                    ...
                                    messageTabClicker.onDeleteClickListener("MessageUnicode/MessageDropById",messageList[position].mESSAGEID)
                                }
                                .setNegativeButton(
                                    "No"
...
//        }else if(adminRole == 1){
            bottomSheetRegionalMessageDetailsAdmin.dismiss()
//        }
    }

    override fun onSuccessMessage(message: String) {
        regionalMessageClickListener.onCreateClick(message)
    }
    override fun onFailedMessage(message: String) {
        regionalMessageClickListener.onFailedMessage(message)
    }
...
//        }else if(adminRole == 1){
        bottomSheetRegionalMessageDetailsAdmin = BottomSheetRegionalMessageDetailsAdmin(this,messageList)
        bottomSheetRegionalMessageDetailsAdmin.show(requireActivity().supportFragmentManager, "TAG")
//        }
    ...
    override fun onUpdateClickListener(messageList: MessageListModel.Message) {
        ...
        bottomSheetUpdateMessage = BottomSheetUpdateRegionalMessage(this,messageList)
    ...
    override fun onDeleteClickListener(url: String, mESSAGEID: Int) {
        ...
        quickRegionalMessageViewModel.getCommonGetFuntion(url,paramsMap)
                        ...
                        Status.SUCCESS -> {
                            ...
                            when {
                                Utils.resultFun(response) == "0" -> {
                                    regionalMessageClickListener.onCreateClick("Deleted Successfully")
                                }
                                else -> {
    ...
    override fun onSubmitUpdateClick(
        ...
        quickRegionalMessageViewModel.getCommonPostFun(url,submitItems)
            .observe(requireActivity(), Observer {
                it?.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
                            ...
                            when {
                                Utils.resultFun(response) == "1" -> {
                                    regionalMessageClickListener.onCreateClick(successMessage)
                                    bottomSheetUpdateMessage.dismiss()
                                }
                                Utils.resultFun(response) == "0" -> {
                                    regionalMessageClickListener.onFailedMessage(failedMessage)
                                }
                                else -> {
                                    regionalMessageClickListener.onFailedMessage(existingMessage)
                                }
                            }
                        }
                        Status.ERROR -> {
...
//
//    fun sendToAllParentStaffPTAConveyor(url : String, paramsMap : Map<String, Int>)
//
//
