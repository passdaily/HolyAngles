package info.passdaily.holy_angles_staff.typeofuser.common_staff_admin.exam_center.progress_card;
...
public class ProgressCardCbseTab extends Fragment{

    String year,exam,sclass,s_cname,s_ename;

    int aCCADEMICID,cLASSID,eXAMID,adminId;
    String cLASSNAME;
    public ProgressCardCbseTab(){}

    public ProgressCardCbseTab(int aCCADEMICID, int cLASSID, int eXAMID, int adminId,String cLASSNAME){
    ...
    public View onCreateView(LayoutInflater layoutInflater,ViewGroup viewGroup,Bundle
        ...
        ProgressCardTableRow_Cbse view=new ProgressCardTableRow_Cbse(getActivity(),year,exam,sclass);
        view.setBackgroundColor(Color.WHITE);
        return view;
    }

   public class ProgressCardTableRow_Cbse extends RelativeLayout{
...
//  http://demostaff.passdaily.in/ElixirApi/Mark/MarkEntryPageLoad?AdminId=1
        ...
        String GET_MarkList= FileUtils.url+"MarkLnvn/ProgressReportLnvn?AccademicId=";
            ...
            for (int x = 0; x<studentname.size(); x++) {
                ...
                for (int y = 0; y < subjectlist.size(); y++) {
                    ...
                    for (int z = 0; z<markList.size(); z++) {
                        ...
                        if (markList.get(z).get("TOTAL_MARK").equals("-1.0")){
                        ...
                        if (studentname.get(x).get("STUDENT_ROLL_NUMBER").equals(markList.get(z).get("STUDENT_ROLL_NUMBER"))){
                            ...
                            if (subjectlist.get(y).get("SUBJECT_ID").equals(markList.get(z).get("SUBJECT_ID"))){
                                ...
                                Log.i(TAG,"TOTAL_MARK "+markList.get(z).get("TOTAL_MARK"));
                                ...
                                for(int c=0;c<subjt.length;c++) {
                                    if (subjt[c].equals(n)){
                                        ...
                                        if (markList.get(z).get("TOTAL_MARK").contains("AB")){
                                            ...
                                            flcnt++;
                                            Float outmark = Float.parseFloat(markList.get(z).get("OUTOFF_MARK"));
                                            outoffmark+=outmark;

                                        }else{
                                            temp= markList.get(z).get("TOTAL_MARK")+" | "+markList.get(z).get("MARK_GRADE")+"~";
                                            Float passmrk = Float.parseFloat(subjectlist.get(y).get("PASS_MARK"));
                                            Float mark = Float.parseFloat(markList.get(z).get("TOTAL_MARK"));
                                            ...
                                            if (markList.get(z).get("MARK_GRADE").contains("A1")){
                                                a1++;
                                            } else if (markList.get(z).get("MARK_GRADE").contains("A2")){
                                                a2++;
                                            }
                                        }
...
/////////////////////////////////////////////percentage
                ...
                float v= (Float.parseFloat(df.format(outc1))*10);
...
//////////////////new corrected
                ...
                float v1= (Float.parseFloat(String.valueOf(outc11))*100);
                int rr_final = (int)Math.round(v1);
...
//                        String.valueOf(new DecimalFormat("##.#").format(tot)),
...
//                sampleObjects1.add(sampleObject1);
                ...
                int outOffMark=outoffmark.intValue();
                ...
                String[] addExtra_String = {
                        ...
                        df1.format(v1)+"%",
                        //    rr_final+"%",
                        "No.of A1 : "+a1 +", No.of A2 : "+a2+",No.of Pass : "+passcnt+", No.of Fail : "+flcnt,
                ...
                for (int l = 0; l<(array3.size()); l++) {
                    ...
                    TableRow.LayoutParams params = new TableRow.LayoutParams(230, LayoutParams.MATCH_PARENT);
...
//                        Collections.addAll(array4, info);
        ...
        public ProgressCardTableRow_Cbse(Context context,String year,String exam,String sclass){
            super(context);
            ...
            GET_MarkList=FileUtils.url+"MarkLnvn/ProgressReportLnvn?AccademicId=" +aCCADEMICID+
...
//                                map3.put("TOTAL_MARK_CE",MARK_ID.getString("TOTAL_MARK_CE"));
        ...
        TableRow componentA_SubWisePass_TableRow(){
            ...
            TableRow.LayoutParams params = new TableRow.LayoutParams(330,LayoutParams.MATCH_PARENT);
            params.setMargins(2,2,0,0);
            TextView textView = this.headerTextView("Sub Wise");
            Typeface typeface = ResourcesCompat.getFont(context, R.font.poppins_bold);
            textView.setTypeface(typeface);
            textView.setTextSize(COMPLEX_UNIT_PX, getResources().getDimension(R.dimen.text_size_02));
            textView.setTextColor(getResources().getColor(R.color.black));
...
//////new Original code
            ...
            TableRow.LayoutParams params = new TableRow.LayoutParams(230,LayoutParams.MATCH_PARENT);
            params.setMargins(2,2,2,0);
            String [] addExtraString={"FL","PAS","STA","TOT","PER","No.of A1","No.of A2"};
            List<String> array3 = new ArrayList<>();
            for(int i=0 ;i<header.length;i++){
