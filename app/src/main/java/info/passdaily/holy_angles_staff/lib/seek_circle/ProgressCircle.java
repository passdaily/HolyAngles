package info.passdaily.holy_angles_staff.lib.seek_circle;
...
public class ProgressCircle extends View {
    ...
    private int mColor1 = Color.parseColor("#05336A");
    private int mColor2 = Color.parseColor("#05336A");
    private int mInactiveColor = Color.parseColor("#8ABFFF");
    ...
    private void initAttributes(Context context, AttributeSet attrs) {
        ...
        try {
            // Read and clamp min
            int min = attributes.getInteger(R.styleable.SeekCircle_minimum, 0);
            mMinProgress = Math.max(min, 0);

            // Read and clamp max
            int max = attributes.getInteger(R.styleable.SeekCircle_maxi, 100);
            mMaxProgress = Math.max(max, 1);

            // Read and clamp progress
            int progress = attributes.getInteger(R.styleable.SeekCircle_progress_, 0);
